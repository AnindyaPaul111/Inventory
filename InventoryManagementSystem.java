...
private static void purchaseProduct(Scanner scanner, List<Product> products,
...
Purchase purchase = new Purchase(product.getName(), quantity, totalPrice, new
Date());
...
FileReader(PURCHASE_HISTORY_FILE))) {
String line;
while ((line = reader.readLine()) != null) {
String[] data = line.split(DELIMITER);
purchaseHistory.add(new Purchase(data[0], Integer.parseInt(data[1]),
Double.parseDouble(data[2]), new Date(Long.parseLong(data[3]))));
}
} catch (IOException | NumberFormatException e) {
...
FileWriter(PURCHASE_HISTORY_FILE))) {
for (Purchase purchase : purchaseHistory) {

writer.write(purchase.getProductName() + DELIMITER + purchase.getQuantity() +
DELIMITER + purchase.getTotalPrice() + DELIMITER +
